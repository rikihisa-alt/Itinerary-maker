    ...
    <header className="fixed top-0 left-0 right-0 z-50 mix-blend-difference pointer-events-none">
      <nav className="max-w-[1440px] mx-auto px-[20px] md:px-[48px] h-[56px] flex items-center justify-between pointer-events-auto">
        ...
        <div className="flex items-center gap-[2px]">
          {[
            { href: "/destinations", l: "観光地" },
            { href: "/itinerary", l: "しおり" },
          ].map((n) => (
            <Link key={n.href} href={n.href} className="px-[12px] py-[6px] text-[13px] text-white/60 hover:text-white transition-colors">{n.l}</Link>
          ))}
          <Link href="/destinations" className="ml-[8px] px-[18px] py-[7px] text-[12px] bg-white text-black rounded-full font-medium hover:bg-white/90 transition-colors">
            旅に出る
          </Link>
        </div>
      </nav>
      ...
      <div className="max-w-[1440px] mx-auto px-[20px] md:px-[48px] py-[64px]">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-[32px] mb-[48px]">
          <div>
            ...
            <p className="text-[12px] leading-[1.8] max-w-[240px]">その一歩が、旅になる。</p>
          </div>
        ...
        <div className="border-t border-white/5 pt-[20px] text-[10px] text-white/10">&copy; 2025 旅のしおり</div>
      </div>
    </footer>
  );
}
