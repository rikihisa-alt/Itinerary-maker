        ...
        <div className="space-y-[40px]">
          ...
          {filtered[0] && (
            <Link href={`/destinations/${filtered[0].id}`} className="group block relative rounded-[8px] overflow-hidden outline outline-1 outline-g2/40" style={{ aspectRatio: "21/9" }}>
              <Image src={filtered[0].images[0]} alt={filtered[0].title} fill className="object-cover transition-transform duration-500 group-hover:scale-[1.03]" sizes="95vw" />
              <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-black/20 to-transparent" />
              <div className="absolute bottom-0 left-0 p-[24px] md:p-[40px] z-10 max-w-[440px]">
                <div className="flex items-center gap-[8px] mb-[6px]">
                  <span className="mono text-[10px] text-white/40 tracking-wider uppercase">{filtered[0].area}</span>
                  <span className="text-[10px] text-white/25">·</span>
                  <span className="mono text-[10px] text-white/25">{filtered[0].stayDuration}min</span>
                </div>
                ...
                <p className="text-[12px] text-white/35 mt-[6px]">{filtered[0].description}</p>
              </div>
            </Link>
          )}

          {/* 2col asymmetric with photos */}
          {filtered.length > 1 && (
            <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-[14px]">
              {filtered.slice(1, 3).map((s, i) => (
                <Link key={s.id} href={`/destinations/${s.id}`}
                  ...
                  <Image src={s.images[0]} alt={s.title} fill className="object-cover transition-transform duration-500 group-hover:scale-[1.05]" sizes={i === 0 ? "60vw" : "35vw"} />
          ...
          {/* Line-separated list with thumbnails */}
          {filtered.length > 3 && (
            <div className="border-t border-g1">
              {filtered.slice(3).map((s) => (
                <Link key={s.id} href={`/destinations/${s.id}`}
                  className="group flex items-center gap-[16px] md:gap-[20px] py-[14px] border-b border-g1/60 hover:bg-white/40 transition-colors px-[4px]">
                  <div className="w-[56px] h-[56px] md:w-[64px] md:h-[64px] rounded-[6px] overflow-hidden shrink-0 relative">
                    <Image src={s.images[0]} alt={s.title} fill className="object-cover" sizes="64px" />
                  </div>
